...
// Init camera
 ...
 //FRAMESIZE_240X240     FRAMESIZE_SVGA  (800 x 600)      FRAMESIZE_VGA(640 x 480)   FRAMESIZE_QVGA  (320 x 240)
  ...
  res = s->set_reg(s, 0x00, 0xFF, 255);     //set gain
  res = s->set_aec_value(s, 274);        //set exposure
...
const int sens_offset_x=23;
const int sens_offset_y=117;
const int sens_size_x=25;
const int sens_size_y=25;
const float sens_spacing=36.5;
...
red=(image_fb->buf[i*2]&0b11111000)>>3;
green=((image_fb->buf[i*2]&0b00000111)<<3)|((image_fb->buf[i*2+1]&0b11100000)>>5);
blue=image_fb->buf[i*2+1]&0b00011111;
intensity[sensor]=intensity[sensor]+red+green+blue;
center_x[sensor]=center_x[sensor]+(red+green+blue)*x;
center_y[sensor]=center_y[sensor]+(red+green+blue)*y;

image_fb->buf[i*2+1]=image_fb->buf[i*2+1]|0x1f;
...
x_pos=x_pos+1;
if (x_pos>320) {x_pos =0; tft.fillRect(0,0,320,100,TFT_WHITE);}
tft.drawLine(x_pos, 100-intensity[1]*3/sens_size_x/sens_size_y, x_pos, 100,TFT_GREEN);
