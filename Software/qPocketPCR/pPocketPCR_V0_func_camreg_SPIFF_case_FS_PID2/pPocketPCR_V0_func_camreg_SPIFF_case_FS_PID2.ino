...
switch (caseUX) {
  case CASE_Main:
         if (ts.touched()) {  
          ...
          if (PointInRect(p,205,70,40,40)) {tft.fillRect(0,0,320,240,TFT_WHITE);  myFileName="/data"  ;myFileName=myFileName+random(1,10000)+".txt";
  x_pos=319;caseUX=CASE_Run;}
...
if (x_pos>280) {x_pos =0; tft.fillRect(0,0,320,(240-60),TFT_WHITE);
 ...
 img.pushImage(0,0, 32, 32, (uint16_t *)buttonStop);
...
myTime_measure=millis();
   ...
   if (ts.touched()) {  
          p = ts.getPoint();
          if (PointInRect(p,320-40,5,32,32)) { setHeater(temperature_mean, 0); draw_main_display();caseUX=CASE_Main;break;}
          
        }
        
    //  setHeater(0, 0);
  sensorValue = analogRead(HEATER_NTC);
...
if (millis()-myTime_measure>1000){
 ...
 File file = FILESYSTEM.open(myFileName, FILE_APPEND);
   if(!file){
  ...
  void setHeater(float temperature,float power)
{
  ...
  analogWrite(HEATER_PWM,heatPower);
  }
  else 
  {
    heatPower=power_cooling(temperature,power);
  analogWrite(FAN_PWM,heatPower);
