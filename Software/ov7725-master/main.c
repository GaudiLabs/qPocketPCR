...
static int ov772x_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_BAND_STOP_FILTER:
		pr_info("V4L2_CID_BAND_STOP_FILTER\n");
		if (!ctrl->val) {
...
static int ov772x_video_probe(struct ov772x_priv *priv)
{
	...
	ov772x_read_reg(client, VER,&ver);
	...
	default:
		...
		ov772x_read_reg(client, MIDL,&midl);
	...
	pr_info("end ov772x_video_probe\n");
	return ret;
}
			...
			const struct i2c_device_id *did)
{
	...
	priv = devm_kzalloc(&client->dev, sizeof(struct ov772x_priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
...
static const struct of_device_id ov772x_camera_of_match[] = {
	{ .compatible = "ovti,ov772x", },
	{},
};
MODULE_DEVICE_TABLE(of, ov772x_camera_of_match);
...
static struct i2c_driver ov772x_i2c_driver = {
	.driver = {
		.name = "ov772x",
		.of_match_table=of_match_ptr(ov772x_camera_of_match),
	},
	.probe    = ov772x_probe,
