...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=captureurs.jpg");
    ...
    if (fb->format == PIXFORMAT_JPEG) {
     ...
     //   Serial.print (fb->buf[i]);
         i++;
         if ((x>100)&&(x<120)&&(y>50)&&(y<70)) fb->buf[i]=fb->buf[i]/2;
          if ((x>200)&&(x<220)&&(y>50)&&(y<70)) fb->buf[i]=fb->buf[i]/2;
    }
   // Serial.println();
}
